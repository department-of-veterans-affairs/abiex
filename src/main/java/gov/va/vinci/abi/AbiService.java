...
import gov.va.vinci.abi.pipeline.Pipeline;
...
public class AbiService {
    ...
    public static void main(String[] args) {
        ...
        try {
            new AbiService().run(configFiles);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void run(File[] configFiles) {
        ...
        try {
            ...
            if (createTypes) {
                pipeline.getLeoTypeSystemDescription().toXML("config/TypeSystem.xml");
                
                File srcDir = new File("generated-types/src");
                srcDir.mkdirs();
                File classesDir = new File("generated-types/classes");
                classesDir.mkdirs();
                pipeline.getLeoTypeSystemDescription().jCasGen(srcDir.getCanonicalPath(), classesDir.getCanonicalPath());
            }
     ...
     * @param service
     */
    private void deleteFilesFromDescriptorDirectory(Service service) {
     ...
     */
    protected Service setServerProperties(Service leoServer) throws MalformedURLException,
